...
class EmailTrackingService
{
    ...
    public function sendTrackedEmailViaGmail(
        ...
        try {
            ...
            try {
                $refreshedToken = $googleService->refreshTokenIfNeeded($token);
                if ($refreshedToken !== $token) {
                    // Token was refreshed, save the new token
                    $user->google_token = json_encode($refreshedToken);
                    $user->save();
                    $token = $refreshedToken;
                }
            } catch (\Exception $refreshError) {
                Log::warning('Failed to refresh Gmail token, using existing token', [
                    'user_id' => $user->id,
    ...
    private function sendViaGmailAPI(Email $email, string $recipientEmail, string $subject, string $body): void
    {
        try {
            ...
            $token = json_decode($adminUser->google_token, true);
            ...
            try {
                ...
                if ($refreshedToken !== $token) {
                    // Token was refreshed, save the new token
                    $adminUser->google_token = json_encode($refreshedToken);
                    $adminUser->save();
                    $token = $refreshedToken;
                }
            } catch (\Exception $refreshError) {
                \Log::warning('Failed to refresh admin Gmail token, using existing token', [
