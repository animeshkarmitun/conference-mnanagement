        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold text-gray-800">Edit ID Card Template</h2>
                    <a href="{{ route('id-cards.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                        Back to Templates
                    </a>
                </div>

                <form action="{{ route('id-cards.update', $idCard) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $idCard->name) }}" required
                            ...
                            <div>
                                ...
                                <select name="type" id="type" required
                                    ...
                                    <option value="participant" {{ old('type', $idCard->type) === 'participant' ? 'selected' : '' }}>Participant</option>
                                    <option value="company_worker" {{ old('type', $idCard->type) === 'company_worker' ? 'selected' : '' }}>Company Worker</option>
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div id="conference-field" class="{{ $idCard->type === 'participant' ? '' : 'hidden' }}">
                                ...
                                <select name="conference_id" id="conference_id"
                                    ...
                                    @foreach($conferences as $conference)
                                        <option value="{{ $conference->id }}" {{ old('conference_id', $idCard->conference_id) == $conference->id ? 'selected' : '' }}>
                                            {{ $conference->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="mt-1 flex items-center space-x-2">
                                    <input type="color" name="background_color" id="background_color" value="{{ old('background_color', $idCard->background_color) }}"
                                        class="h-10 w-20 border border-gray-300 rounded">
                                    <input type="text" value="{{ old('background_color', $idCard->background_color) }}" 
                            ...
                            <div>
                                ...
                                <div class="mt-1 flex items-center space-x-2">
                                    <input type="color" name="text_color" id="text_color" value="{{ old('text_color', $idCard->text_color) }}"
                                        class="h-10 w-20 border border-gray-300 rounded">
                                    <input type="text" value="{{ old('text_color', $idCard->text_color) }}"
                                        class="flex-1 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                        placeholder="#000000">
                                </div>
                                @error('text_color')
                            ...
                            <div>
                                ...
                                <div class="mt-1 flex items-center space-x-2">
                                    <input type="color" name="accent_color" id="accent_color" value="{{ old('accent_color', $idCard->accent_color) }}"
                                        class="h-10 w-20 border border-gray-300 rounded">
                                    <input type="text" value="{{ old('accent_color', $idCard->accent_color) }}"
                                        class="flex-1 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                        placeholder="#007bff">
                                </div>
                                @error('accent_color')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="flex items-center">
                                <input type="checkbox" name="include_photo" id="include_photo" value="1" 
                                    {{ old('include_photo', $idCard->include_photo) ? 'checked' : '' }}
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" name="include_qr_code" id="include_qr_code" value="1"
                                    {{ old('include_qr_code', $idCard->include_qr_code) ? 'checked' : '' }}
                                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
