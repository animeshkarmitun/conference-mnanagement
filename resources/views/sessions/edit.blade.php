    ...
    <form method="POST" action="{{ route('sessions.update', $session) }}">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="room" id="room" value="{{ old('room', $session->room) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
                @error('room')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label for="capacity" class="block text-sm font-medium text-gray-700">Capacity</label>
                <input type="number" name="capacity" id="capacity" value="{{ old('capacity', $session->capacity) }}" required min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
        ...
        <div class="mb-6">
            ...
            <select name="participants[]" id="participants" multiple class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
                @foreach($participants as $participant)
                    <option value="{{ $participant->id }}" {{ in_array($participant->id, old('participants', $session->participants->pluck('id')->toArray())) ? 'selected' : '' }}>
        ...
        <div class="flex justify-end">
            ...
            <button type="submit" class="bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-lg font-semibold">Update Session</button>
        </div>
    </form>
</div>
