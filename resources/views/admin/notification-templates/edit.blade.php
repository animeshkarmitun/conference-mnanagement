...
<div class="py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="mt-2 text-3xl font-bold text-gray-900">Edit Notification Template</h1>
            <p class="mt-2 text-gray-600">Update the notification message template for {{ $notificationTemplate->type_name }}</p>
        </div>

        <form action="{{ route('admin.notification-templates.update', $notificationTemplate) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="px-6 py-4 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="mt-1 p-3 bg-gray-50 rounded-md border border-gray-200">
                                <span class="text-sm font-medium text-gray-900">{{ $notificationTemplate->type_name }}</span>
                            </div>
                            <p class="mt-1 text-sm text-gray-500">Notification type cannot be changed after creation.</p>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="2"
                                  ...
                                  placeholder="Optional description for this template">{{ old('description', $notificationTemplate->description) }}</textarea>
                        @error('description')
                    ...
                    <div>
                        ...
                        <textarea name="message_template" id="message_template" rows="4"
                                  ...
                                  required>{{ old('message_template', $notificationTemplate->message_template) }}</textarea>
            ...
            <div class="flex items-center justify-end space-x-3">
                ...
                <a href="{{ route('admin.notification-templates.preview', $notificationTemplate) }}" 
                   class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
