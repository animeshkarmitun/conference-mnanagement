...
<div class="rounded-2xl bg-gradient-to-r from-blue-100 via-blue-50 to-white shadow flex items-center justify-between px-8 py-6 mb-10 border border-blue-200">
    <div class="flex items-center">
        ...
        <div>
            <h1 class="text-3xl font-extrabold text-blue-800 tracking-tight mb-1">Itineraries</h1>
            <div class="text-gray-600 text-lg font-medium">View and export participant travel details</div>
        </div>
    </div>
    <div>
        <a href="{{ route('event-coordinator.export-itinerary') }}" class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold text-lg shadow-lg transition duration-200 transform hover:scale-105">
...
<div class="bg-white rounded-xl shadow p-6 mb-8">
    ...
    <form method="GET" action="{{ route('event-coordinator.itineraries') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
        ...
        <div class="flex items-end">
            ...
            <a href="{{ route('event-coordinator.itineraries') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg font-semibold">Clear</a>
        </div>
    </form>
</div>
...
<div class="bg-white rounded-xl shadow p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="text-sm text-gray-500">{{ $itineraries->total() }} records found</div>
    </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div>
                                    <div class="text-sm font-medium text-gray-900">
                                        {{ $itinerary->participant->user->first_name ?? $itinerary->participant->user->name }} {{ $itinerary->participant->user->last_name ?? '' }}
                                    </div>
                                    <div class="text-sm text-gray-500">{{ $itinerary->participant->user->email }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    {{ $itinerary->participant->conference->name ?? 'N/A' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $itinerary->hotel->name ?? 'No hotel assigned' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($itinerary->arrival_date)
                                    <div>{{ date('M d, Y', strtotime($itinerary->arrival_date)) }}</div>
                                    <div class="text-xs text-gray-500">{{ date('H:i', strtotime($itinerary->arrival_date)) }}</div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($itinerary->departure_date)
                                    <div>{{ date('M d, Y', strtotime($itinerary->departure_date)) }}</div>
                                    <div class="text-xs text-gray-500">{{ date('H:i', strtotime($itinerary->departure_date)) }}</div>
                                @else
                                    <span class="text-gray-400">Not specified</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if($itinerary->flight_info)
                                    <div class="max-w-xs truncate" title="{{ $itinerary->flight_info }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $itinerary->extra_nights ?? 0 }} nights
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $itinerary->participant->organization ?? 'N/A' }}
                            </td>
                        </tr>
                    @endforeach
                </tbody>
