...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow p-6">
    ...
    <form method="POST" action="{{ route('tasks.update', $task) }}">
            ...
            <div>
                ...
                <select name="conference_id" id="conference_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
                    ...
                    @foreach($conferences as $conference)
                        <option value="{{ $conference->id }}" {{ old('conference_id', $task->conference_id) == $conference->id ? 'selected' : '' }}>{{ $conference->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="mb-4">
            ...
            <input type="hidden" name="assigned_to" id="assigned_to_input" value="{{ old('assigned_to') ? (is_array(old('assigned_to')) ? implode(',', old('assigned_to')) : old('assigned_to')) : $task->users->pluck('id')->implode(',') }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize with existing task assignments or old values
    const initialValues = assignedToInput.value;
    if (initialValues) {
        const userIds = initialValues.split(',');
