<form method="POST" action="{{ route('participants.travel.update', $participant) }}" enctype="multipart/form-data" class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="datetime-local" name="arrival_date" value="{{ old('arrival_date', optional($travelDetail)->arrival_date ? \Carbon\Carbon::parse($travelDetail->arrival_date)->format('Y-m-d\TH:i') : '' ) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 required-field">
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700">Departure Date</label>
            <input type="datetime-local" name="departure_date" value="{{ old('departure_date', optional($travelDetail)->departure_date ? \Carbon\Carbon::parse($travelDetail->departure_date)->format('Y-m-d\TH:i') : '' ) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 required-field">
        </div>
    </div>
            <div>
            <label class="block text-sm font-medium text-gray-700">Flight Info</label>
            <textarea name="flight_info" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 required-field">{{ old('flight_info', optional($travelDetail)->flight_info) }}</textarea>
        </div>
    <div>
        <label class="block text-sm font-medium text-gray-700">Hotel</label>
        <div class="flex gap-2 items-center">
            <select name="hotel_id" id="hotel-select" class="mt-1 block flex-1 rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500 required-field">
